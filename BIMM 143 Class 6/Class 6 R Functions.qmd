...
Let's start sumple and write our first function to add some numbers.
...
Hmm.. this sucks! I need to try something else and come back to this issue of misisng values (NAs)
...
I want to find the location of the minumum value not the value itself. For this I can use `which.min()`
```{r}
student1
which.min(student1)
```
...
To use `apply()` function on this `gradebook` dataset I need to decide whether i want to "apply" the `grade()` function over the row or colums of the `gradebook`.
```{r}
...
Or
I want to `apply()` the `cor()` function over the `masked_gradebook` and use the `finalgrade` scores for the class
